...
def wsgi_handler(environ, start_response):
	...
	if(uri.startswith(base_path)):
		environ['dathomir.path'] = uri[len(base_path):]
	else:
	...
	result = _wsgi_handle_file(req)
	if(result):
		content = []
		if(result[1]):
			response_headers.append(('Content-Type', result[1]))
			response_headers.appen(('Content-Length', result[2]))
			try:
	...
	response_headers.append(('Content-Type', rsrc.get_content_type(environ)))
	content = rsrc.get_content(environ)
	response_headers.append(('Content-Length', len(content)))
